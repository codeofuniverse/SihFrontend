...
const CounsellorDashboard = (meeturl) => {
  const { user } = useAuth();
  ...
  const fetchSessions = async () => {
    try {
      ...
      setStats({
        ...
        upcomingSessions: counsellorSessions.filter(
          (s) =>
            s.status === "scheduled" && new Date(s.session_date) > new Date()
        ).length,
        completedSessions: counsellorSessions.filter(
          (s) => s.status === "completed"
        ).length,
        todaySessions: counsellorSessions.filter(
          (s) => format(new Date(s.session_date), "yyyy-MM-dd") === todayStr
        ).length,
      });
    } catch (error) {
  ...
  const createAndJoinMeeting = (meeturl) => {
                      ...
                      {getTodaySessions().map((session) => (
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Button
                              onClick={() =>
                                createAndJoinMeeting(session.session_url)
                              }
                              size="sm"
                            >
                              Join Session
              ...
              <div className="space-y-4">
                <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    "Very helpful session. Dr. clerk provided excellent coping
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    "Great listener and provided practical advice. Feeling much
                    better after our session."
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    - Anonymous Student
                  </p>
